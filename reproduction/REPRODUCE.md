...
| Figures 1–2 `itagState = 0x100A` for NTDS.dit (page 4) | Manual hex inspection at file offset `0x18000` (page 4 = 3 × 32768) of `samples/identity/2025/clean/ntds.dit` | Bytes 34–35 = `0A 10` (little-endian = `0x100A`; upper nibble `0x1` = ctagReserved, lower 12 bits = 10 tags) |
...
```powershell
# WS2025 NTDS.dit page 4 (offset 0x18000 = 3 x 32768)
Get-Content samples/identity/2025/clean/ntds.dit -AsByteStream -ReadCount 0 |
  Select-Object -First 98344 |
  Select-Object -Skip 98304 |    # offset 0x18000
...
- **The `dissect.esedb` PR #46 baseline.** The PR is open-source at https://github.com/fox-it/dissect.esedb/pull/46. Unlike Impacket's `page_size > 8192` gate and our format-revision gate, PR #46 applies the `& 0x0FFF` mask **unconditionally** (no page-size or format-revision guard); you can confirm this by reading `page.py` in the PR. The consequence is not a parse failure: the unconditional mask lets `dissect.esedb` read the WS2025 16 KiB and 4 KiB samples in this record, but the same mask is **unsafe** on dense legacy 32 KiB pages, where a legitimately large tag count (`>= 0x1000`) would be silently truncated. Our claim is therefore about *safety*, not completeness — only the revision gate is simultaneously complete on WS2025 and safe on legacy pages (see the tools comparison in the paper).
