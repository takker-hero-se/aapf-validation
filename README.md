...
1. **Format-revision-gated patch validation** — applying the `itagState` mask gated on the ESE format revision (`>= 0x0122`, not on a `page_size >= 32768` gate) resolves WS2025 16 KiB `DataStore.edb` and 4 KiB `SRUDB.dat` parsing failures that page-size-gated community fixes (e.g., dissect.esedb PR #46, Impacket) leave unaddressed. After we reported the defect, upstream `libesedb` incorporated an equivalent revision-gated fix in **release 20260704** (commit `768a0474`); these patches reproduce the paper's pinned build.
